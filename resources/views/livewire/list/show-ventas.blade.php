        ...
        <div class="bg-blue-600 px-6 py-4 flex items-center justify-between">
            <h2 class="text-2xl font-bold text-white flex items-center gap-2">
                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M3 3h18v18H3V3z M8 8h8v8H8V8z" />
                </svg>
                            ...
                            @foreach ($ver_venta->detalles as $detalle)
                                <tr class="hover:bg-gray-50 transition">
                                    <td class="p-3 text-sm text-gray-700">
                                        ...
                                        @if ($detalle->variante)
                                            <span class="text-xs text-gray-500"> -
                                                {{ $detalle->variante->descripcion_variante }}</span>
                                        @endif
                                    </td>
                                    <td class="p-3 text-sm text-gray-700">
                                        @if ($detalle->cantidad_decimal)
                                            {{ $detalle->cantidad_decimal }} <span
                                                class="text-xs text-gray-500">{{ $detalle->unidad_medida_venta ?? 'kg' }}</span>
                                        @else
                                            {{ $detalle->cantidad }}
                                        @endif
                                    </td>
                                    <td class="p-3 text-sm text-gray-700">
                                        ...
                                        @if ($detalle->unidad_medida_venta)
                                            <span
                                                class="text-xs text-gray-500">/{{ $detalle->unidad_medida_venta }}</span>
                                        @endif
                                    </td>
                                    <td class="p-3 text-sm text-gray-700 font-semibold">
                                        ${{ $detalle->cantidad * $detalle->precio_venta }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
