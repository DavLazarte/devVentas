@if ($row->tiene_variantes)
    <div class="space-y-1">
        ...
        <button onclick="toggleVariantes({{ $row->idarticulo }})" class="text-xs text-blue-600 hover:text-blue-800 block">
            Ver detalles
        </button>
        <div id="variantes-{{ $row->idarticulo }}" class="hidden mt-2 text-xs">
            @foreach ($row->variantes->take(3) as $variante)
                <div class="text-gray-600">
                    {{ $variante->sku }}: ${{ $variante->precio_unitario }}
                    @if ($row->tipo_venta === 'peso' || $row->tipo_venta === 'volumen')
                        ({{ number_format($variante->stock_decimal, 3) }} {{ $row->unidad_medida }})
                    @else
                        ({{ $variante->stock }})
                    @endif
                </div>
