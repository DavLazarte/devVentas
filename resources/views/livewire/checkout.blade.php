    ...
    <main class="pb-24">
        ...
        <div class="bg-white mb-2">
            <div class="px-4 py-3 border-b border-gray-200">
                ...
                <p class="text-sm text-gray-600">{{ count($items) }}
                    {{ count($items) === 1 ? 'elemento' : 'elementos' }}</p>
            </div>
                                            ...
                                            @if (isset($item['fecha_servicio']) && isset($item['hora_inicio']))
                                                <div class="mt-2 p-2 bg-purple-50 rounded-md">
                                                    <div class="flex items-center space-x-1 text-purple-700">
                                                        ...
                                                        <span class="text-xs font-medium">Fecha:
                                                            {{ \Carbon\Carbon::parse($item['fecha_servicio'])->format('d/m/Y') }}</span>
                                                    </div>
                                                    <div class="flex items-center space-x-1 text-purple-700 mt-1">
                                                        ...
                                                        <span class="text-xs font-medium">Hora:
                                                    ...
                                                    @if (isset($item['duracion']))
                                                        <div class="flex items-center space-x-1 text-purple-700 mt-1">
                                                            ...
                                                            <span class="text-xs font-medium">Duración:
                                                                {{ $item['duracion'] }} min</span>
                                                        </div>
                                                    @endif
                                                    @if (isset($item['empleado_nombre']))
                                                        <div class="flex items-center space-x-1 text-purple-700 mt-1">
                                                            ...
                                                            <span class="text-xs font-medium">Profesional:
                                                                {{ $item['empleado_nombre'] }}</span>
                                                        </div>
                                                    @endif
                                                </div>
                                            @else
                                                <div class="mt-2 p-2 bg-blue-50 rounded-md">
                                                    <span class="text-xs text-blue-700 font-medium">Servicio sin turno
                                                        fijo</span>
                                                </div>
                                            @endif
                                        @endif
        ...
        <div class="bg-white px-4 py-4 mb-2">
            ...
            <form wire:submit.prevent="procesarPedido" class="space-y-4">
                <!-- Name -->
                <div>
                    <label for="nombre_cliente" class="block text-sm font-medium text-gray-700 mb-1">Nombre
                        completo</label>
                    <input type="text" id="nombre_cliente" wire:model.defer="nombre_cliente"
                ...
                @if ($tieneProductos)
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            <label for="codigo_postal" class="block text-sm font-medium text-gray-700 mb-1">Código
                                Postal</label>
                            <input type="text" id="codigo_postal" wire:model.defer="codigo_postal"
                                placeholder="1234"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-purple-500 focus:border-purple-500">
                            @error('codigo_postal')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                    ...
                    <!-- Delivery Notes -->
                    <div>
                        <label for="notas_entrega" class="block text-sm font-medium text-gray-700 mb-1">Notas de
                            entrega (opcional)</label>
                        <textarea id="notas_entrega" wire:model.defer="notas_entrega" rows="3"
                ...
                @else
                    ...
                    <div>
                        <label for="notas_entrega" class="block text-sm font-medium text-gray-700 mb-1">Notas
                ...
                <div class="space-y-3">
                    ...
                    <label
                        ...
                        <input type="radio" wire:model="metodo_pago" value="efectivo"
                    ...
                    <label
                        class="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                        <input type="radio" wire:model="metodo_pago" value="tarjeta"
                            class="text-purple-600 focus:ring-purple-500">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-600">Puede contener incremento el precio del
                                    producto/servicio</p>
                            </div>
                        </div>
                    </label>
                ...
                <div class="flex items-start space-x-3">
                    <input type="checkbox" id="terms" wire:model="acepto_terminos" required
                        class="mt-1 text-purple-600 focus:ring-purple-500 rounded">
                    <div>
                        <label for="terms" class="text-sm text-gray-700 cursor-pointer">
                            Acepto los <a href="{{ url('/terminos') }}" class="text-purple-600 underline">términos y
                                condiciones</a> y la
                            <a href="{{ url('/politicas') }}" class="text-purple-600 underline">política de
                                privacidad</a>
                        </label>
                ...
                <!-- Submit Button -->
                <button type="submit"
                    class="w-full py-3 px-4 rounded-lg font-medium text-lg transition-colors 
           {{-- 💡 Cambia el color del botón cuando está cargando para indicar que está inactivo --}}
           bg-purple-600 hover:bg-purple-700 
           wire:loading.attr='disabled' 
           wire:loading.class='bg-purple-400 cursor-not-allowed'">

                    {{-- 1. Loader y texto de carga (visible SÓLO mientras se procesa) --}}
                    <span wire:loading>
                    ...
                    {{-- 2. Texto normal del botón (oculto MIENTRAS se procesa) --}}
