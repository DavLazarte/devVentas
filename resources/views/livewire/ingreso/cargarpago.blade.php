...
<div class="flex space-x-1 justify-around">
    {{-- @dd($row) --}}
    <button wire:click="mostrarPagos({{ $row->{'persona.idpersona'} }})" wire:loading.attr="disabled"
        wire:loading.class="opacity-50 cursor-not-allowed" wire:target="mostrarPagos"
        class="flex items-center bg-green-500 hover:bg-green-600 text-white text-sm font-bold py-2 px-4 rounded-full relative">
        ...
        <!-- Loader animado -->
        <svg wire:loading wire:target="mostrarPagos" class="animate-spin h-4 w-4 ml-2 text-white"
    ...
    @if ($row->estado === 'Activo')
        <button wire:click="$emit('confirmarCancelacionVentaDeuda', {{ $row->id }})"
